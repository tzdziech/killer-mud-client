...
public sealed record ArtifactTryProgress(int Number, int Completed, int Total)
{
    public string DisplayText => $"try {Number} ({Completed}/{Total})";
}

/// <summary>
/// Coordinates the "/mapuj &lt;liczba&gt;" conversation: sends "try 1" through "try &lt;liczba&gt;"
/// one at a time, awaiting each response in full before sending the next, and captures each raw
/// response verbatim. Structurally this is the same capture engine as
/// <see cref="AbilityMappingCoordinator"/> (see that type's docs for why) — duplicated rather than
/// shared, matching how <see cref="RareCatalogRefreshCoordinator"/> and
/// <see cref="BookCatalogRefreshCoordinator"/> each keep their own copy instead of a common base.
/// </summary>
public sealed class ArtifactTryMappingCoordinator
{
    ...
    public ArtifactTryMappingCoordinator(TimeSpan? quietPeriod = null, TimeSpan? responseTimeout = null)
    {
    ...
    /// <summary>
    /// Sends "try 1" through "try &lt;count&gt;", in order, awaiting each response in full before
    /// sending the next. <paramref name="onEntryCaptured"/>, when given, is awaited right after
    /// each capture with the full list captured so far — a run can be a hundred-plus round-trips,
    /// so without this a disconnect or crash partway through would discard everything captured so
    /// far instead of letting the caller persist as it goes.
    /// </summary>
    public async Task<IReadOnlyList<ArtifactTryEntry>> RunAsync(
        int count,
        Func<string, CancellationToken, Task> sendCommandAsync,
        IProgress<ArtifactTryProgress>? progress = null,
        CancellationToken cancellationToken = default,
        Func<IReadOnlyList<ArtifactTryEntry>, CancellationToken, Task>? onEntryCaptured = null)
    {
        ArgumentNullException.ThrowIfNull(sendCommandAsync);
        ArgumentOutOfRangeException.ThrowIfLessThan(count, 1);

        var captured = new List<ArtifactTryEntry>(count);
        for (var number = 1; number <= count; number++)
        {
            cancellationToken.ThrowIfCancellationRequested();
            progress?.Report(new ArtifactTryProgress(number, number - 1, count));
            ...
            captured.Add(new ArtifactTryEntry
            {
                Number = number,
                RawText = RareListParser.ExtractDetailText(lines),
        ...
        progress?.Report(new ArtifactTryProgress(count, count, count));
        return captured;
    }
    ...
    private CaptureSession BeginCapture()
    {
        ...
        lock (_captureLock)
        {
            ...
            {
                throw new InvalidOperationException("Inne mapowanie artefaktów jest już aktywne.");
            }
