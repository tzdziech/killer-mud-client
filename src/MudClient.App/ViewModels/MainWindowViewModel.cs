...
public sealed class MainWindowViewModel : ObservableObject, IAsyncDisposable
{
    ...
    // Set to true temporarily when diagnosing a server-response capture problem.
    private static readonly bool ShowEquipmentScanDiagnostics = false;
    ...
    // TCP can split the pager marker between receives. Keep only a short tail so every active
    ...
    private readonly StringBuilder _activeSelfExamineResponse = new();
    private readonly object _silentEquipmentResponseLock = new();
        ...
        SkillProgressHistoryStore? skillProgressHistoryStore = null)
    {
        ...
        EquipmentInventory = new EquipmentInventoryViewModel();
    ...
    public EquipmentInventoryViewModel EquipmentInventory { get; }
    ...
    {
        // Profile activation must not stop the disk-sync timer that drives rule hot-reload.
        _timers.CancelAll(MultiboxSyncTimerName);
        ...
        AutowalkLocation entry, IReadOnlySet<int>? excludedRoomIds = null, MapPath? precomputedPath = null)
    {
        ...
        PaintRoute(path, 0);
        // SendAutowalkStep owns position recovery: sitting/sleeping wait for confirmation,
        // resting can move immediately, and fighting pauses without a stray stand command.
        SendAutowalkStep();
    }
    ...
    private async Task SendStuckStepRecoveryCommandsAsync(int step, CancellationToken cancellationToken)
    {
        try
        {
            if (_autowalkPath is { } path && step < path.Steps.Count)
            {
                ...
                if (PauseAutowalkDoorCommandsForCombat()) return;
    ...
    private void BeginAutowalkStandRecovery()
    {
        ...
        AutowalkStatusText = $"Postać siedzi lub śpi — wstaję i wznawiam trasę do „{_autowalkTargetName}”.";
    ...
    private void ReflectMergedEntries(
        ...
        bool isGlobal)
    {
        ReloadRulesOnly(mergedRules, mergedFolders, isGlobal);
    ...
    /// <summary>Adaptation of laszlowaty's rule-only reload to JSON profiles and multibox merging.
    /// The caller has already resolved local/disk conflicts. Never rebuild or synchronize timers.
    /// Keep unchanged rule instances (including open editors), replacing only changed entries.</summary>
    private void ReloadRulesOnly(
        IReadOnlyList<ProfileRule> rules, IReadOnlyList<ProfileFolder> folders, bool isGlobal)
    {
        var wasSuppressed = _suppressTreeRebuild;
        _suppressTreeRebuild = true;
        try
        {
            var byId = rules.ToDictionary(rule => rule.Id, StringComparer.Ordinal);
            foreach (var entry in AutomationRules.Where(rule => rule.IsGlobal == isGlobal).ToList())
            {
                if (!byId.TryGetValue(entry.Id, out var rule)) AutomationRules.Remove(entry);
                else if (JsonSerializer.Serialize(ToProfileRule(entry)) != JsonSerializer.Serialize(rule))
                    AutomationRules[AutomationRules.IndexOf(entry)] = MakeRuleEntry(rule, isGlobal);
                byId.Remove(entry.Id);
            }
            foreach (var rule in byId.Values) AutomationRules.Add(MakeRuleEntry(rule, isGlobal));

            var ruleFolders = folders.Where(folder => folder.Kind is FolderKind.Aliases or FolderKind.Triggers)
                .ToDictionary(folder => folder.Id, StringComparer.Ordinal);
            foreach (var entry in Folders.Where(folder => folder.IsGlobal == isGlobal &&
                         folder.Kind is FolderKind.Aliases or FolderKind.Triggers).ToList())
            {
                if (!ruleFolders.TryGetValue(entry.Id, out var folder)) Folders.Remove(entry);
                else if (JsonSerializer.Serialize(ToProfileFolder(entry)) != JsonSerializer.Serialize(folder))
                    Folders[Folders.IndexOf(entry)] = MakeFolderNode(folder, isGlobal);
                ruleFolders.Remove(entry.Id);
            }
            foreach (var folder in ruleFolders.Values) Folders.Add(MakeFolderNode(folder, isGlobal));
        }
        finally
        {
            _suppressTreeRebuild = wasSuppressed;
        }
    }
    ...
    /// <summary>Live Room.People choices for the Inventory "give" submenu. The player character
    /// is excluded; every other server-reported room person remains selectable.</summary>
    public IReadOnlyList<RoomPerson> InventoryGiveTargets => _latestRoomPeople
        .Where(person => !string.Equals(person.Name, _latestCharacterName, StringComparison.OrdinalIgnoreCase))
    ...
    /// <summary>Runs typed commands and equipment-menu commands through one pipeline, preserving
    /// alias processing, history, terminal echo and normal MUD error handling.</summary>
    private async Task SendCommandAsync(string sourceCommand)
    {
    ...
    [
        ...
        new("selfheal", "Autoleczenie", "Leczenie poniżej progu HP także bez aktywnej farmy.",
            () => AutoSelfHealEnabled, v => AutoSelfHealEnabled = v),
    ...
    private void StartEquipmentMonitoringForCurrentSession()
    {
        ...
        AnnounceInitialEquipmentScanStage("pokój");
    ...
    private bool HandleEquipmentInventoryResponse(string text)
    {
        ...
        if (_activeSelfExamine)
        {
            ...
            if (!ContainsPromptAfterMarker(_activeSelfExamineResponse.ToString(), " uzywa:")) return !ShowEquipmentScanDiagnostics;
        ...
        if (_activeEquipmentExamineKey is { } examineKey)
        {
            ...
            if (!EquipmentInventorySnapshotParser.ContainsPrompt(_activeEquipmentExamineResponse.ToString())) return !ShowEquipmentScanDiagnostics;
        ...
        if (Volatile.Read(ref _hiddenEquipmentResponsePending) == 1)
        {
            ...
            {
                _equipmentNamesToExamine = equipment.Select(item => AnsiText.StripKillerColors(AnsiText.StripAnsi(item.Name)))
            ...
            if (_initialEquipmentLoadAnnounced)
            {
                ...
                AnnounceInitialEquipmentScanStage("ekwipunek");
        ...
        {
            _manualInventoryResponse.Append(text);
            if (TryContinueEquipmentPager(text)) return false;
            if (!EquipmentInventorySnapshotParser.ContainsPrompt(_manualInventoryResponse.ToString())
                || !EquipmentInventorySnapshotParser.TryParseInventory(_manualInventoryResponse.ToString(), out inventory))
            {
                return false;
            }
            Interlocked.Exchange(ref _manualInventoryResponsePending, 0);
            _manualInventoryResponse.Clear();
        }
        else
        {
            return false;
        ...
        return isSilentInventoryResponse && !ShowEquipmentScanDiagnostics;
    }
    ...
    {
        var lowItems = snapshot.Equipment.Select(item => (Source: $"E:{item.Location}", item.Name))
            .Concat(snapshot.Inventory.Select(item => (Source: "I", item.Name)))
            .Select(item => (item.Source, item.Name, Percent: EquipmentInventorySnapshotParser.GetDurabilityPercent(item.Name)))
            .Where(item => item.Percent is < 30)
            .ToArray();
        var currentKeys = lowItems.Select(item => $"{item.Source}:{EquipmentInventorySnapshotParser.WithoutDurabilityPercent(item.Name)}")
            .ToHashSet(StringComparer.OrdinalIgnoreCase);
        foreach (var item in lowItems.Where(item => ! _lowDurabilityWarningKeys.Contains($"{item.Source}:{EquipmentInventorySnapshotParser.WithoutDurabilityPercent(item.Name)}")))
        {
            var itemName = EquipmentInventorySnapshotParser.WithoutDurabilityPercent(item.Name);
            Dispatcher.UIThread.Post(() => EmitSystem($"[Ekwipunek] Uwaga: {itemName} ma tylko {item.Percent}% trwałości.", 31));
        }
        _lowDurabilityWarningKeys = currentKeys;
    }
            ...
            if (_initialEquipmentLoadAnnounced)
            {
                switch (_initialEquipmentScanStage)
                {
                    ...
                    case InitialEquipmentScanStage.InventoryExamine:
                        _initialEquipmentScanStage = InitialEquipmentScanStage.Tattoos;
                        AnnounceInitialEquipmentScanStage("tatuaże");
    ...
    private async Task SendEquipmentPagerContinueAsync()
    {
        ...
        {
            Interlocked.Exchange(ref _equipmentPagerContinuePending, 0);
        }
    }

    /// <summary>Continues one server-paged response without discarding the accumulated text.
    /// This is shared by all Equipment panel readers because the server uses the same Enter
    /// marker for long tables, examines, room output and tattoos.</summary>
    private bool TryContinueEquipmentPager(string text)
    {
        _equipmentPagerDetectionBuffer.Append(text);
        if (!EquipmentInventorySnapshotParser.ContainsPagerPrompt(_equipmentPagerDetectionBuffer.ToString()))
        {
            const int tailLength = 64;
            if (_equipmentPagerDetectionBuffer.Length > tailLength)
            {
                _equipmentPagerDetectionBuffer.Remove(0, _equipmentPagerDetectionBuffer.Length - tailLength);
            }
            return false;
        }
    ...
    private static bool IsEquipmentChangeLine(string line)
    {
        var key = AnsiText.StripKillerColors(AnsiText.StripAnsi(line)).ToLowerInvariant();
        return key.Contains("zakladasz") || key.Contains("wsuwasz") || key.Contains("zdejmujesz") || key.Contains("przestajesz uzywac")
            || key.Contains("bierzesz do reki") || key.Contains("bierzesz ") && key.Contains(" w druga reke")
            || key.Contains("chwytasz do reki") || key.Contains("chwytasz ") && key.Contains(" w obie rece")
            || key.Contains("pewnie chwytasz ") || key.Contains("rozbraja cie");
    }

    private static bool IsEquipmentAdditionLine(string line)
    {
        var key = AnsiText.StripKillerColors(AnsiText.StripAnsi(line)).ToLowerInvariant();
        return key.Contains("zakladasz") || key.Contains("wsuwasz") || key.Contains("bierzesz do reki")
            || key.Contains("bierzesz ") && key.Contains(" w druga reke") || key.Contains("chwytasz do reki")
    ...
    private bool CaptureRoomGroundItems(string text)
    {
        ...
        if (_initialEquipmentLoadAnnounced && _initialEquipmentScanStage == InitialEquipmentScanStage.Room)
        {
            ...
            AnnounceInitialEquipmentScanStage("inventory");
            _ = RequestInventorySilentlyAsync();
            return suppressTerminal;
        }
    ...
    private void OnTextReceived(string text)
    {
        ...
        // Catalog refreshers own their own paged requests. Any other server list (including a
    ...
    {
        if (ConsumeSilentEquipmentLine(line))
        {
            return;
        }
        ...
        if (_equipmentInventoryCheckedForSession && IsEquipmentChangeLine(line))
        {
            ...
            if (!_refreshExamineOnlyNewEquipment)
            {
                // Removing or losing an item still needs fresh eq/inv state, but must never cause
                // another automatic examine. Empty means "intentionally examine no rows"; null is
                // reserved for the initial full scan after login.
                _equipmentNamesToExamine = [];
                _pendingEquipmentExamines.Clear();
            }
            _ = RefreshEquipmentInventorySilentlyAsync();
    ...
    /// <summary>Text arrives immediately before its completed lines in <see cref="MudSession"/>.
    /// Remember those lines at the text boundary so the later line pipeline cannot run triggers,
    /// automations, statistics or combat parsing for an automatic equipment response.</summary>
    private void RememberSilentEquipmentLines(string text)
    {
        lock (_silentEquipmentResponseLock)
        {
            foreach (var line in text.Split('\n').Select(value => value.TrimEnd('\r')))
                if (line.Length > 0) _silentEquipmentResponseLines.Add(line);
        }
    }
    ...
    private Task ExecuteInventorySpellIdentifyAsync(EquipmentInventoryRow? row)
    {
        ...
        if (CurrentIdentifySpellState != IdentifySpellState.Ready)
        {
            EmitSystem("[Ekwipunek] Znasz czar 'identify', ale nie masz go teraz zapamiętanego.", 33);
            return Task.CompletedTask;
        }
    ...
    private bool ConsumeSilentEquipmentLine(string line)
    {
        lock (_silentEquipmentResponseLock)
        {
            return _silentEquipmentResponseLines.Remove(line);
        }
    }
    ...
    private void UpdateCharacterPosition(string position)
    {
        ...
        var wasSitting = AutowalkRecoveryPolicy.RequiresStandBeforeMovement(_latestCharacterPosition);
        ...
        var nowSitting = AutowalkRecoveryPolicy.RequiresStandBeforeMovement(position);
    ...
    private void OnAutowalkCombatEnded()
    {
        Dispatcher.UIThread.Post(() =>
        {
            ...
            _autowalkPausedForCombat = false;
            if (_autowalkIsFollowingLeader && _latestGroupUpdate is { } group &&
                ShouldAutoFollowLeader(AutoFollowLeaderEnabled, IsConnected, true, true,
                    _autowalkFollowTargetVnum, _latestCharacterPosition, group,
                    _latestCharacterName, Map.CurrentVnum, out _))
            {
                TryAutoFollowLeader(group);
                return;
            }
    ...
    private void OnAutowalkRestingStarted()
    {
        Dispatcher.UIThread.Post(() =>
        {
            // An order to rest must not strand a follower on its existing route.
            // Explicit manual navigation retains its pause-until-standing behavior.
            if (_autowalkIsFollowingLeader || _autowalkPath is null || _autowalkStep >= _autowalkPath.Steps.Count)
            {
                return;
            }
    ...
    private bool PauseAutowalkDoorCommandsForCombat()
    {
    ...
    private void OnGmcpReceived(GmcpMessage message)
    {
        ...
        var wasRecording = IsRecordingSession;
        _combatCapture.ObserveGmcp(message);
        if (wasRecording != IsRecordingSession)
        {
            Dispatcher.UIThread.Post(NotifySessionRecordingChanged);
        }
    ...
    private void UpdateLeaderRoomTrail(CharacterGroupUpdate update)
    {
        ...
        if (leader is null)
        {
            ...
            _leaderRoomTrailLeaderName = null;
            return;
        }
        ...
        if (string.IsNullOrWhiteSpace(leader.Room)) return;
    ...
    private void TryAutoFollowLeader(CharacterGroupUpdate update)
    {
        ...
        {
            if (string.Equals(Map.CurrentVnum, target.Vnum, StringComparison.Ordinal))
            {
                StopAutowalk("Lider wrócił do Twojego pokoju — podążanie zakończone.");
                return;
            }
            ...
            _autowalkIsFollowingLeader = _autowalkPath is not null &&
                string.Equals(_autowalkPath.To.Vnum, target.Vnum, StringComparison.Ordinal);
            _autowalkFollowTargetVnum = _autowalkIsFollowingLeader ? target.Vnum : null;
            if (!_autowalkIsFollowingLeader && _autowalkPath is not null)
            {
                StopAutowalk("Autofollow: brak trasy do nowej pozycji lidera.", "error");
            }
        }
    }
    ...
    private AsyncRelayCommand? _toggleSessionRecordingCommand;
    public AsyncRelayCommand ToggleSessionRecordingCommand =>
        _toggleSessionRecordingCommand ??= new AsyncRelayCommand(async () =>
        {
            if (IsRecordingSession) await StopTelnetLineCaptureAsync();
            else StartTelnetLineCapture();
        });

    private void NotifySessionRecordingChanged()
    {
